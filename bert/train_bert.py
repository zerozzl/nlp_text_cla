...
from utils.dataloader import TextDataset, load_tag_dict
from utils.logger import Logger
from utils import modelloader, evaluator


def get_dataset(args, tokenizer):
    train_dataset = TextDataset('%s/%s/char/train.txt' % (args.data_path, args.task), 'char', tokenizer,
                                args.max_inp_size, do_pad=True, do_to_id=True, for_bert=True, debug=args.debug)
    test_dataset = TextDataset('%s/%s/char/test.txt' % (args.data_path, args.task), 'char', tokenizer,
                               args.max_inp_size, do_pad=True, do_to_id=True, for_bert=True, debug=args.debug)
...
def main(args):
    ...
    if args.pretrained_model_path is not None:
        ...
        model = Bert('%s/config.json' % args.pretrained_bert_path, '%s/pytorch_model.bin' % args.pretrained_bert_path,
                     len(tag_to_id), args.bert_freeze)
        model = model.cpu() if args.use_cpu else model.cuda()
